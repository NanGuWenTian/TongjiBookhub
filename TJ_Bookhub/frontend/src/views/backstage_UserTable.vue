<template>
  <div class="table-page">
    ...
    <formModal
      v-if ="showModal"
      :formTitle="isEdit ? '编辑活动种类' : '新增活动种类'"
...
const fetchCategoriesData = async (page = 1) => {
  ...
  try {
    ...
    console.error('获取活动种类数据失败:', error);
  } finally {
    loading.value = false;
  }
};
...
const handleFormSubmit = async (payload) => {
  try {
    let response;
    if (payload.isEdit) {
      response = await axios.put(`/api/event_category/${payload.recordId}`, payload.data);
      console.log('更新活动种类成功:', response.data);
    } else {
      response = await axios.post('/api/event_category', payload.data);
      console.log('创建活动种类成功:', response.data);
    }
...
const deleteCategory = async (id) => {
  if (confirm('确定要删除这个活动种类吗？')) {
    try {
      await axios.delete(`/api/event_category/${id}`);
      console.log('删除活动种类成功');
      fetchCategoriesData(currentPage.value);
    } catch (error) {
      console.error('删除活动种类失败:', error);
    }
  }
};
